
import { useCallback, useEffect, useRef } from 'react';

type SoundType = 'shoot' | 'explosion' | 'gameOver' | 'start' | 'speedUp';

export const useSound = () => {
  const audioRefs = useRef<Record<SoundType, HTMLAudioElement | null>>({
    shoot: null,
    explosion: null,
    gameOver: null,
    start: null,
    speedUp: null
  });

  // Initialize audio elements
  useEffect(() => {
    // Create audio elements for each sound
    // Replaced the shoot sound with a more sci-fi laser sound
    audioRefs.current.shoot = new Audio('data:audio/wav;base64,');
    audioRefs.current.explosion = new Audio('data:audio/wav;base64,');
    audioRefs.current.gameOver = new Audio('data:audio/wav;base64,');
    
    // New sounds
    audioRefs.current.start = new Audio('data:audio/wav;base64,');
    audioRefs.current.speedUp = new Audio('data:audio/wav;base64,');

    // Clean up function to release audio resources
    return () => {
      Object.values(audioRefs.current).forEach(audio => {
        if (audio) {
          audio.pause();
          audio.src = '';
        }
      });
    };
  }, []);

  const playSound = useCallback((type: SoundType) => {
    const audio = audioRefs.current[type];
    if (audio) {
      // Reset and play
      audio.currentTime = 0;
      audio.play().catch(err => {
        console.log(`Error playing ${type} sound:`, err);
      });
    }
  }, []);

  return { playSound };
};
